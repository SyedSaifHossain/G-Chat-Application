...
class GroupMessageAdapter(
            ...
            when (message.type) {
                ...
                "voice" -> {
                    ...
                    binding.sentVoicePlayButton.setOnClickListener {
                        playVoiceMessage(message, adapterPosition)
                    }
                }
                "file" -> {
    ...
    private fun playVoiceMessage(message: GroupMessage, position: Int) {
        if (isPlaying) {
            stopVoiceMessage()
        }
        ...
        message.audioUrl?.let { audioUrl ->
            try {
                ...
                Toast.makeText(context, "播放语音消息", Toast.LENGTH_SHORT).show()
            } catch (e: Exception) {
